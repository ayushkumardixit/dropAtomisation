...
rmin, rmax, zmin, zmax = [-Ldomain, Ldomain, 1.5, 9.5]
lw = 2

folder = 'Video_lengthpoints'  # output folder
    ...
    if not os.path.exists(place):
        ...
        if os.path.exists(name):
            ...
            if (len(segs) == 0):
                ...
                ax.set_title(r'$tU_0/R_0$ = %4.1f' % t, fontsize=TickLabel)
                #plt.scatter(segs[0], segs[1])
                #print("The line collection array: ",segs)

                # Plot the points
                ax.plot(ymintemp, xmintemp, 'o', color='red', markersize=10)
                ax.plot(ymaxtemp, xmaxtemp, 'o', color='blue', markersize=10)
                ax.plot()
                ## Copied Lines
                ax.set_aspect('equal')
